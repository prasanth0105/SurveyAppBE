...
const joimiddleware = require("../middlewares/validators/joimiddleware");
  ...
  if (password === confirmPassword) {
    bcrypt.genSalt(10, async (_err, salt)=> {
      bcrypt.hash(newUser.password, salt, async (_err, hash)=> {
        ...
        try {
          const userId = transaction.insert("user", {username: req.body.username, email: req.body.email, password: newUser.password});
...
const loginData=(req, res, _next)=> {
  User.find({username: req.body.username}, (_err, users)=> {
...
router.post("/register", joimiddleware(joiSchema.registerInfo), (req, res, next) => registerData(req, res, next));
router.post("/login", joimiddleware(joiSchema.loginInfo), (req, res, next) => loginData(req, res, next));
