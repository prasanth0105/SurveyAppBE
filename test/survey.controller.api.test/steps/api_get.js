...
let API = 'http://localhost:8080';
  ...
  When('I make a GET request to {stringInDoubleQuotes}', function (endPoint) {
    return fetch(API+endPoint)
      .then(res => res.json())
      .then(function (body) {this.body=body});
  });
  Then(/^For ([^"]*) response property ([^"]*) should be ([^"]*)$/, function (index,prop,val) {
  ...
  When('I make a GET request to {stringInDoubleQuotes} with that survey ID.', function (endPoint) {
